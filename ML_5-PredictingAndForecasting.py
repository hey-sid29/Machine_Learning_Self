...
#Now for future purposes for any kind of machine learning model; We got to figure out the meaningful columns of data
#So we have to work out which data means the most; in this example we did take Adj.Close, HL_PCT, PCT_Change, Adj. Volume
#But what all data means the most:

#         Price        x           x           somewhat useful

df= df[['Adj. Close', 'HL_PCT', 'PCT_Change', 'Adj. Volume']]

#this means that other than the price and the volatility(to some extent) are useful


df.fillna(value=-9999, inplace=True)
forecast_col= 'Adj. Close'
forecast_out= int(math.ceil(0.1* len(df)))   #10% of the data for predicting
